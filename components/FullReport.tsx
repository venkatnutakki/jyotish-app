...
export function FullReport({ data }: { data: ReportData }) {
  ...
  // Reflect the ayanāṃśa the chart was actually cast in — not a hardcoded
  // "Lahiri", which was wrong whenever the user picked Raman or KP.
  const ayanamsaName = AYANAMSA_INFO[chart.birth.ayanamsa ?? "lahiri"].name;
    ...
    <div className="report space-y-8 text-sm">
      ...
      <header className="break-inside-avoid text-center">
        ...
        <p className="text-xs text-amber-100/40">
          ...
          {chart.birth.tzOffsetHours} · Ayanāṃśa ({ayanamsaName}) {chart.ayanamsa.toFixed(4)}°
        </p>
      </header>
      ...
      <footer className="border-t border-white/10 pt-3 text-center text-xs text-amber-100/40">
        Generated by Jyotish · Sidereal ({ayanamsaName}) · Whole-sign houses (KP uses
